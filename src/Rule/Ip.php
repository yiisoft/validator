...
final class Ip implements SerializableRuleInterface, SkipOnErrorInterface, WhenInterface, SkipOnEmptyInterface
{
    ...
    /**
     * @param array<string, list<string>> $networks Custom network aliases, that can be used in {@see $ranges}:
     *
     *  - key - alias name.
     *  - value - array of strings. String can be an IP range, IP address or another alias. String can be negated with
     * {@see NEGATION_CHAR} (independent of {@see $allowNegation} option).
     *
     ...
     * @param bool $allowSubnet Whether the address can be an IP with CIDR subnet, like `192.168.10.0/24`. The following
     * values are possible:
     *
     * - `false` - the address must not have a subnet (default).
     * - `true` - specifying a subnet is optional.
     * @param bool $requireSubnet
     * @param bool $allowNegation Whether address may have a {@see NEGATION_CHAR} character at the beginning. Defaults
     * to `false`.
     * @param string $message User-defined error message is used when validation fails due to the wrong IP address
     * format.
     *
     ...
     * @param string $ipv4NotAllowedMessage User-defined error message is used when validation fails due to the disabled
     * IPv4 validation when {@see $allowIpv4} is set.
     *
     * You may use the following placeholders in the message:
     *
     * - `{attribute}`: the label of the attribute being validated.
     * - `{value}`: the value of the attribute being validated.
     * @param string $ipv6NotAllowedMessage User-defined error message is used when validation fails due to the disabled
     * IPv6 validation when {@see $allowIpv6} is set.
     *
     * You may use the following placeholders in the message:
     *
     * - `{attribute}`: the label of the attribute being validated.
     * - `{value}`: the value of the attribute being validated.
     * @param string $wrongCidrMessage User-defined error message is used when validation fails due to the wrong CIDR
     * when {@see $allowSubnet} is set.
     *
     * You may use the following placeholders in the message:
     *
     * - `{attribute}`: the label of the attribute being validated.
     * - `{value}`: the value of the attribute being validated.
     * @param string $noSubnetMessage User-defined error message is used when validation fails due to
     * {@see $allowSubnet} is used, but the CIDR prefix is not set.
     *
     * You may use the following placeholders in the message:
     *
     * - `{attribute}`: the label of the attribute being validated.
     * - `{value}`: the value of the attribute being validated.
     * @param string $hasSubnetMessage User-defined error message is used when validation fails due to
     * {@see $allowSubnet} is false, but CIDR prefix is present.
     *
     * You may use the following placeholders in the message:
     *
     * - `{attribute}`: the label of the attribute being validated.
     * - `{value}`: the value of the attribute being validated.
     * @param string $notInRangeMessage User-defined error message is used when validation fails due to IP address

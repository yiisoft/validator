...
class FileTest extends TestCase
{
    ...
    {
        $val = new File(['extensions' => 'jpeg, jpg, gif']);
        $this->assertEquals(['jpeg', 'jpg', 'gif'], $val->extensions);

        $val = new File(['extensions' => 'jpeg']);
        $this->assertEquals(['jpeg'], $val->extensions);

        $val = new File(['extensions' => '']);
    ...
    {
        $validator = new File(['minFiles' => 2, 'maxFiles' => 0]);
    ...
    {
        $validator = new File(['minFiles' => 2, 'maxFiles' => 4]);
        $model = FakedValidationModel::createWithAttributes(
            [
                'attr_images' => $this->createTestFiles(
                    [
    ...
    public function testValidateAttribute()
    {
        ...
        $val = new File(['skipOnEmpty' => false]);
        $m = $this->createModelForAttributeTest();
        $val->validateAttribute($m, 'attr_files');
        $this->assertFalse($m->hasErrors());
        $val->validateAttribute($m, 'attr_files_empty');
    ...
    {
        $validator = new File(['mimeTypes' => $mask]);
    ...
    {
        $validator = new File(['extensions' => (array) $allowedExtensions]);
        $file = $this->getRealTestFile($fileName);
        $filePath = $this->app->getAlias('@yii/tests/framework/validators/data/mimeType/') . $fileName;

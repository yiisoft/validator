...
По умолчанию, даже если при проверке свойства возникает ошибка, обрабатываются все последующие правила в наборе.
...
По умолчанию, отсутствующие/пустые значения свойств проверяются. Если значение отсутствует, предполагается, что оно равно `null`.
Если вы хотите, чтобы свойство было необязательным, используйте `skipOnEmpty: true`.
...
Если свойство является обязательным, более уместно использовать `skipOnError: true` вместе с предшествующим правилом `Required`, вместо `skipOnEmpty: true`.
...
- Когда `true`, `Yiisoft\Validator\EmptyCondition\WhenEmpty` автоматически используется в качестве обратного вызова - только переданное (соответствующее свойство должно присутствовать) и непустое значение (не `null`, `[]`, или `''`) проверяется.
...
Вот пример, где значение пусто, только если оно отсутствует (при использовании свойств) или равно нулю:
...
Метод `$context->getDataSet()->getPropertyValue()` позволяет вам получить значение любого другого свойства в рамках `ValidationContext`:
